...
interannuelle inférée de l'indice. Plusieurs approches ont été expérimentées, telles que la réalisation de la régression sur des champs à plus haute
...
Poisson. Dans un premier temps, nous ajouterons explicitement un diagnostic du mode de variabilité El Niño comme nouveau prédicteur dans la régression. L'ENSO
...
une source de prévisibilité de l'activité cyclonique à l'échelle interannuelle. L'ajout d'un diagnostic ENSO dans la régression est considérée ici à titre
...
régression de Poisson, dans le but de voir si cet apport d'information est susceptible d'améliorer la variabilité interannuelle inférée de l'indice.
...
sur le diagnostic du jet subtropical (STJ), décrit et illustré dans la \cref{sec:filtrage_mid_latitudes} (\cref{chap:chapitre_2}). Ce filtre est choisi ici
...
%    quatre colonnes correspond à une saison. La première ligne présente le diagnostic pour l'hémisphère nord, tandis que la seconde ligne le présente pour
%    l'hémisphère sud.}
%    \label{fig:STJ_PRE625REFT359x}
%\end{figure}

\subsubsection{diagnostic ENSO}\label{sec:diag_enso}

Le mode de variabilité El Niño (respectivement La Niña) est caractérisé par une anomalie chaude (respectivement froide) de SST dans l'océan Pacifique central.
...
augmenter la fréquence d'occurrence des TC dans certains bassins, notamment dans le Pacifique, et tend à la réduire dans l'océan Indien, et vice versa. Dans
...
résolution et l'ONI, agrégé annuellement de la même façon. Précisons que, à l'instar de toutes les opérations de calcul de variabilité interannuelle réalisées
...
calibration de l'indice régional sur la fréquence annuelle moyenne inférée de son homologue global. L'indice régional est alors nommé LOAM10, ou simplement
...
absolue est celui présentant la plus grande différence passant d'environ \num{1.40} pour M10 à \num{2.44} pour AM10. Une valeur si élévée de $b_\eta$ est
...
modifier la variabilité interannuelle inférée des indices de cyclogénèse d'une part, et à améliorer la corrélation entre cette variabilité et celle mesurée
...
la plus grande amplitude. Inversement, les bassins où l'activité inférée des indices est faible tendent à présenter une faible amplitude.

\subsection{Discussion}

Cette section s'est concentrée sur l'introduction d'un diagnostic du mode de variabilité El Niño dans la construction d'un indice de cyclogénèse par
...
simulation, lesquelles étant préalablement filtrées par le diagnostic du jet subtropical présenté dans le \cref{chap:chapitre_2}. La répartition spatiale de
...
\cref{sec:tracking_arpege}). L'utilisation du diagnostic ENSO dans la construction d'indices de cyclogénèse consiste à apporter artificiellement, et de la
...
efficacement à améliorer la représentation de la variabilité interannuelle inférée de l'indice de cyclogénèse régional, avec l'impact le plus frappant
...
artificiel de l'ONI, construit en répliquant la valeur (scalaire) du diagnostic en tout point de l'espace, ne contient par définition aucune variabilité
...
et la variabilité détectée dans la plupart des bassins. Chercher à améliorer la représentation de la variabilité interannuelle inférée d'un indice de
...
En effet, puisque les coefficients expriment des sensibilités pour des changements unitaires, l'application d'un facteur d'échelle au prédicteur se répercute de
la même manière sur son coefficient dans la régression. En outre, et bien que \textcite{camargo_testing_2014} ne précisent pas l'unité utilisée pour le déficit
...
nombre annuel de TC inféré de l'indice AVPD et celui mesuré par le traqueur est généralement inchangée par rapport à AM10, mais peut également être dégradée,
...
\begin{figure}[htb]
    ...
    \caption{Corrélations croisées entre le nombre annuel de TC simulé par les indices AVPD ; AM10 ; M10 du \cref{chap:chapitre_3} appliqué à la simulation ARPEGE, et du
...
majorité des cas, l'absence de tendance dans la variabilité inférée des indices AVPD et EVPD (l'équivalent du AVPD sur ERA5) est en accord avec les
...
\textcite{cattiaux_projected_2020,chauvin_future_2020}, nous appliquons comme précédemment le filtre du STJ aux quatre jeux de cyclogénèses, en ne conservant
...
sur la saison SON dans la simulation ATL-F est rehaussée de \ang{0.6} par rapport à celle évaluée sur ATL-P. Spécifiquement, la latitude moyenne du STJ sur SON
...
indices aux simulations RCP8.5, les coefficients de calibration utilisés pour le climat présent sont réemployés tels quels. Ces coefficients sont précisés dans
...
extrêmement robuste aux perturbations de leurs coefficients, une première partie s'est consacrée à l'introduction d'un diagnostic représentatif de l'ENSO
...
la variabilité inférée de ceux-ci. L'introduction explicite d'un indice ENSO dans la régression laisse alors espérer une réponse plus consistante aux phases de
...
des bassins WPac et SPac. Cela se traduit par un intervalle de confiance à \prct{95} pour les coefficients $b$ associés à l'ONI à incluant \num{0}, ou autrement
...
interannuelle inférée du nouvel indice régional s'en trouve néanmoins sensiblement améliorée. En particulier, la corrélation entre le nombre annuel de TC
simulé et celui mesuré par le schéma de détection est rendue statistiquement significative dans tous les bassins océaniques au seuil de \prct{95} lorsque
...
inférée des indices. L'utilisation de modèles statistiques plus sophistiqués pourrait peut-être apporter une solution à ce problème, tels que les modèles
