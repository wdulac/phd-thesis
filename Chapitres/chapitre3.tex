...
\textquote{quasi-Poisson} utilisée par \textcite{tippett_poisson_2011}.
...
matrice $\mathbf{x}$ de taille $\num{82 944} \times \num{5}$. Le second indice, nommé C10, est quant à lui établi toujours sur les moyennes climatologiques,
mais avec une résolution de \ang{1.0} ($m = \num{518400}$). Le troisième, M25 est construit sur les moyennes mensuelles ($N_{\mathrm{temps}} = \num{480}$) à
\ang{2.5}, et donc avec $m = \num{3317760}$. Enfin le dernier, M10, correspond par conséquent, selon la nomenclature utilisée jusqu'à maintenant, à une
régression faite sur les moyennes mensuelles ERA5 à une résolution de \ang{1.0}, conduisant à la plus grande matrice $\mathbf{x}$, caractérisée par $m =
\num{20736000}$. Notons qu'au pas de temps mensuel, la taille de la matrice $\mathbf{x}$ est le facteur limitant la résolution spatiale. C'est là une des
raisons faisant qu'ERA5 n'est pas utilisée à sa pleine résolution de \ang{0.25}, puisque la régression sur un nombre de réalisations $m$ plus grand
nécessiterait des ressources de calcul supplémentaires.
...
Cela signifie que les indices sont évalués sur les mêmes données ayant servi à ajuster la relation, ce qui est d'autant plus vrai pour M10. On ne se préoccupe
pas de répartir l'ensemble des données entre un jeu d'apprentissage et un jeu d'évaluation dans la mesure où le surapprentissage ne constitue ici pas un risque,
mais au contraire une propriété désirable.
...
\prct{95}, où sens où les pourcentiles \prct{2.5} et \prct{97.5} sont de même signe que la moyenne estimée.

\begin{table}[htpb]
    \centering
    \caption{Estimation de la moyenne des coefficients des régressions de Poisson établies entre les quatre variables de grande échelle du TCS, issues d'ERA5,
    et l'activité observée issue d'IBTrACS entre 1980 et 2019.}
    ...
    \begin{tabular}{crrrrrrrr}
       ...
       \midrule
              & $b_0$ & $b_{\eta}$ & $b_{V_{\mathrm{shear}}}$ & $b_H$ & $b_T$ & Spatiale     & Temporelle & $m$ \\
       \midrule
