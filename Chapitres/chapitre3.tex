...
d'une qualité légèrement supérieure. L'intérêt principal réside cependant dans l'interprétabilité facilitée par la SST relative, le PI n'étant à priori pas
...
%n'est pas assez prononcée. Le TCS simule également mieux l'étendue méridienne de l'activité cyclonique par rapport au GPI ---~notamment grâce à l'usage de la
...
par des prédicteurs dynamiques. Spécifiquement, les prédicteurs sélectionnées sont le cisaillement vertical, la composante verticale du vent à \hPa{500}, la
...
\cref{fig:produit_ingredients} (\vpageref{fig:produit_ingredients}) de la \cref{sec:climato_ingredients} (\cref{chap:chapitre_1}). Cette dernière présentait
...
terre-mer utilisé, mais les points visibles indiquent tout de même une activité non négligeable. Ces régions, larges d'à peine plus de \km{200}, ne peuvent
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\textwidth]{acgi_mean_IBTrACS_season.png}
...
\subsubsection*{Distribution méridienne}

La \cref{fig:tcs_cygp_gpi_zonal} présente la distribution méridienne de l'activité pour les trois indices explicités dans la \cref{sec:tour_horizon_indices} et
...
\begin{figure}[htb]
    ...
    résolution horizontale, entre 1981 et 2019. Le tracé en tirets indique la distribution méridienne annuelle moyenne des premières observations des systèmes
...
observations sont en bon accord pour ce qui concerne l'amplitude de l'activité méridienne moyenne ainsi que sa position. Le maximum des observations est
...
méridienne des trois indices ne correspondent aux observations. Ils présentent tous un biais équatorial plus ou moins prononcé, avec une différence de \ang{4}
...
meilleure distribution méridienne.
...
\begin{figure}[tb]
    ...
    \caption{Cycle annuel de l'activité cyclonique inférée des trois indices de cyclogénèses définis dans la \cref{sec:tour_horizon_indices} sur
...
juillet~--~août~--~septembre. Dans les deux bassins de l'hémisphère sud, l'activité inférée du TCS est au contraire plus faible que pour les deux autres
...
\ang{90} et \ang{135}E. La limite méridienne demeure inchangée, entre \ang{0} et \ang{60}S. Ce découpage est plus proche du découpage pratiqué pour la
...
inférée des indices est, de manière générale, nettement moins proche d'IBTrACS avant l'année \num{1993}. Notons en effet que la corrélation interannuelle
...
observations. Dans l'hémisphère nord, la répartition méridienne est plus contrastée et tous les indices présentent un biais équatorial plus ou moins
...
la variabilité interannuelle inférée des indices apparaît ici comme un élément central pour espérer comprendre la divergence dans les projections futures
...
cyclonique en climat futur, on peut aussi se demander quelle confiance accorder dans la tendance du changement de l'activité cyclonique inférée d'un indice de
...
inférée des indices (spécifiquement du YGP et CYGP) pour les résolutions les plus hautes, amenant alors \textcite{camargo_tropical_2007} à spéculer
qu'une résolution accrue pourrait s'accompagner d'une réponse plus réaliste dans les variations interannuelles du nombre de TC inféré des indices. Si les
...
construirons un indice de cyclogénèse régional, ou plus exactement un indice pseudo-global défini comme une agrégation d'indices régionaux ---~également
...
mensuelle de ces champs, issus de la réanalyse ERA5. Cela revient donc à recalculer les coefficients $b$ issus de \textcite{tippett_poisson_2011} et donnés dans
la \cref{sec:modelisation_stat}, qui furent déterminés avec la climatologie mensuelle des prédicteurs des réanalyses NCEP
...
\textquote{observation}, dans ce contexte, est à comprendre au sens statistique du terme, et ne présage en rien de si les cyclogénèses sont réellement des
...
\num{12}$. Toutefois, l'emploi d'un masque terre-mer approprié réduit la taille des vecteurs, car les points de terre masqués sont ignorés. Celui-ci est par
ailleurs choisi de façon à être très conservatif. Spécifiquement, toute fraction de mer strictement inférieure à \prct{100} est masquée. Le masque est ensuite
interpolé de manière bi-linéaire à la résolution spatiale souhaitée, avant d'être appliqué aux variables de grande échelle. La taille effective,
...
prédicteurs, si bien que l'écart entre les régressions faites ici et le TCS pour le terme $b_0$ serait principalement la conséquence de l'utilisation d'une
...
valider les coefficients obtenus pour C10, M25 et M10, moyennant l'effet susmentionné de la période considérée.
...
une moyenne de \num{1.3863} pour M25 et M10, soit une différence de $+$\num{0.3243}, autrement dit une augmentation de \prct{30.5} du coefficient $b_{\eta}$ par
rapport au TCS.
...
résolution plus fine pour un indice construit selon la méthodologie de \textcite{tippett_poisson_2011} comme la moyenne des deux différences. Le même constat est
...
La \cref{fig:my_fit_meridional} présente la distribution méridienne des quatre indices nouvellement construits, ainsi que celles du TCS et d'IBTrACS, faites
...
climatologiques. Le tracé de C10 est superposé à C25, indiquant que la résolution accrue ne suffit pas à modifier la distribution méridienne moyenne, en dépit
...
Il est immédiatement clair sur la \cref{fig:variabilite_basin_5_indices} que la variabilité interannuelle inférée des quatre indices du
...
la corrélation est systématiquement de \num{0.99}. Les corrélations entre les paires d'indice de même agrégation temporelle (climatologique ou mensuelle) mais
...
\subsubsection{Apport de l'utilisation de coefficients réajustés}\label{sec:diagnostic_rang}
...
méridienne moyenne, en particulier dans la région équatoriale (voir \cref{fig:my_fit_meridional}), on peut alors se demander si ces coefficients sont porteurs
...
Pour cela, on introduit un nouveau diagnostic indiquant la moyenne et médiane de la distribution des rangs de la valeur prise par un indice, au point de
...
cyclogénèses\footnote{\num{2789} cyclogénèses sur \num{40} années fait une moyenne bien différente de celle de \num{84.7} TC par an évoquée à plusieurs reprises
dans ce chapitre. La raison à cela provient du fait que certaines observations sont perdues en raison du masque terre-mer assez conservatif utilisé, puisque
l'indice n'est pas défini en ces points.} issues d'IBTrACS entre \num{1980} et \num{2019}, toutes régions confondues.
...
interannuelle inférée des mêmes indices l'est quant à elle très fortement. En effet, ni la résolution horizontale accrue, ni l'introduction de variabilité
...
\cref{sec:diagnostic_rang} que l'estimation de nouveaux coefficients $b$, supposément mieux adaptés au jeu de donnée cible, apporte tout de même une
plus-value. Le nouveau diagnostic qui y est introduit, consistant en la mesure des paramètres de la distribution des rangs des valeurs prises par l'indice aux
...
L'humidité relative à \hPa{600} fait partie des variables pour lesquelles la sensibilité de la fréquence attendue de cyclogénèses est relativement stable pour
...
atteinte pour le EPac avec \num{0.0748}. Cette plage de valeur correspond donc à une sensibilité de l'ordre de \prct{6.4} à \prct{7.5} pour un changement
...
le bassin NInd contribue à augmenter le $b_H$ global, ce bassin n'étant pas intégré aux régression locales pour cause de données observationelles manquantes.
L'erreur standard relative de $b_H$ pour les régression régionales se situe entre \prct{4} et \prct{9} de la moyenne estimée.
...
entre l'activité observée et l'indice ONI (\textit{Oceanic Niño Index}) ---~diagnostic opérationnel du mode de variabilité El Niño/Niña, défini comme
...
Le bassin NAtl est caractérisé par un fort déport de l'activité inférée de M10 à proximité de l'équateur dans tout le reste du bassin, à l'exception du Golfe
du Mexique (Encadré en rouge, GoM), qui voit lui aussi une diminution relative dans LM10. Rappelons que la \cref{sec:apport_res_temporel_spatial} a mis en
évidence le rôle du coefficient $b_\eta$ dans le décalage méridional de l'activité inférée. Dans la mesure où LM10 présente un $b_\eta$ encore plus important,
il est permis de penser que c'est cette sensibilité accrue à la vorticité absolue de LM10 qui est à l'origine de la diminution de l'activité sur la bande entre
...
NAtl de la \cref{fig:diff_LM10_M10}. Dans ces deux sous-régions, les différences sont toutefois marginales, de l'ordre d'environ $10^{-1}$ TC par an. On peut
éventuellement noter que dans la MDR, la fréquence annuelle simulée par LM10 est légèrement plus proche d'IBTrACS qu'elle ne l'est dans l'indice global M10.
L'origine de la diminution (relative) dans le GoM est incertaine, mais au vu des coefficients du \cref{tab:coefs_fit_bassin}, il semble probable que ce soit
principalement le résultat de la baisse de sensibilité à la SST relative, et peut-être aussi dans une moindre mesure à la diminution de la sensibilité à
l'humidité relative (comparativement moindre), dans la mesure où la diminution de la sensibilité au cisaillement vertical ne saurait expliquer une baisse de
l'activité. Notons que la racine carrée de l'erreur quadratique moyenne (\textit{Root Mean Squared Error}, RMSE) entre la moyenne annuelle d'IBTrACS ---~lissée
au préalable avec un filtre gaussien à \ang{1} d'écart-type~--- et la moyenne annuelle de l'indice passe de \num{3.3e-3} avec M10 à \num{3.1e-3} avec LM10,
indiquant donc dans une meilleure (marginalement) proximité de LM10 avec IBTrACS.
...
précédentes montrant toutes deux une distribution méridienne de l'activité moyenne plus éloignée des observations dans l'hémisphère nord que dans l'hémisphère
sud.
...
de l'indice. Il n'est par conséquent pas attendu que LM10 se démarque de M10 ou encore du TCS pour ce qui est de la corrélation entre la variabilité
...
caractérisé par ses coefficients $b$, eux-mêmes étant fonction de la région. Cet indice étant, outre sa dépendance régionale, construit selon la même
...
meilleure ressemblance avec les observations (à l'exception notable du GoM dans le bassin NAtl). Les changements dans la répartition méridienne sont donc
...
de la répartition méridienne de l'activité cyclonique dans l'hémisphère nord est à l'origine de la mauvaise distribution méridienne simulée dans cet
...
%M10. Dans le bassin WPac, l'activité cyclonique inférée de LM10 est au contraire déportée vers l'équateur, par le biais notamment d'une baisse importante de la
%sensibilité à la vorticité. Rappelons en effet que la sensibilité à la vorticité a pour conséquence notable de piloter la répartition méridienne de l'activité
...
indices considérés surestiment l'activité dans l'hémisphère sud et la sous-estiment dans l'hémisphère nord. De plus, la distribution méridienne de l'activité
...
proche sur les deux tableaux. Tous les indices présentent par ailleurs une activité simulée trop importante au voisinage de l'équateur. Dans l'ensemble, le
...
interannuelles. Seules les régions EPac et NAtl présentent une bonne corrélation entre la variabilité simulée et celle observée. Dans le bassin EPac, les séries
...
$b$ de la régression se traduit par une modification de la répartition spatiale de l'activité cyclonique inférée de l'indice. Il est montré que l'effet est
...
inférée des indices nouvellement construits ne s'en retrouve que très peu affectée. Les variabilités interannuelles des quatre indices définis dans la
...
dernière doit être contrôlée par le choix des prédicteurs utilisés. Mais malgré l'absence d'amélioration, le diagnostic introduit dans la
\cref{sec:diagnostic_rang}, basé sur la mesure de la distribution des rangs des valeurs prises par l'indice aux points de grille correspondant aux
cyclogénèses observées, montre que la détermination de coefficients mieux adaptés à un jeu de donné cible possède tout de même un intérêt. Il est en effet
...
l'origine de la difficulté rencontrée par tous les indices de cyclogénèse construits à l'échelle globale à simuler une distribution méridienne réaliste.
