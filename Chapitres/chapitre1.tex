...
\subsubsection{Approche indirecte : Indices de cyclogénèses}\label{sec:intro_indices}
...
échelle via les variables, ou prédicteurs, associés à cette activité. Pour ce faire, la façon la plus commune consiste à utiliser un indice de cyclogénèse
...
\begin{align*}
    ...
    \rho &= \underbrace{\vphantom{X_1}A}_{\text{Calibration}} \times \underbrace{X_1}_{\text{Thermique}} \times \underbrace{X_2}_{\text{Dynamique}}
\end{align*}
%
...
prédicteurs. Le SGP était exprimé en TC par maille de \ang{5}$\times$\ang{5} par 20 ans et était capable de reproduire fidèlement la densité de cyclones observés sur
la période \num{1952}~--~\num{1971}.
...
fois le SGP, ainsi qu'une variante annuelle appelée YGP, dans une expérience \ensuremath{2\times\ce{CO2}} sur des simulations de \num{15} ans. Leurs travaux
