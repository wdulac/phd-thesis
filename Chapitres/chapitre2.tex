...
ainsi que les perturbations baroclines en bornant la vitesse du vent d'ouest à \hPa{200} au dessus du centre du cyclone à \ms{5}. Dans
...
le profil vertical de vent, en imposant que le vent moyen à \hPa{850} supérieur à la moyenne du niveau à \hPa{300}, toujours dans la boîte centrée. Beaucoup
...
sur lequel elles sont établies, en particulier si les TC n'y sont pas explictement assimilés.
...
\end{itemize}
%
Rappelons que que le paramètre $V_U^T$, de l'espace des phases de \textcite{hart_cyclone_2003}, est défini comme le gradient vertical entre \hPa{600} et
\hPa{300} de la perturbation de hauteur géopotentielle maximale dans un rayon de \km{500} centré sur le cyclone. Une valeur $V_U^T < 0$, est indicative de la
présence d'un vent thermique (induit par un gradient de température) plus fort à \hPa{600} qu'à \hPa{300}. Ce paramètre permet alors de discriminer les cyclones
...
faux-positivité intégré sur la période \num{1981}~--~\num{2019} et sur les cinq bassins océaniques d'intérêt s'élève à \prct{24} (\cref{sec:papier_results}).
Lorsque le filtre du $V_U^T$ est désactivé (jeu 2), cette quantité vaut alors \prct{60}. Si ce filtre est si efficace, c'est en effet parce que la grande
...
%La quantité de ces derniers est en partie un effet
%secondaire de l'optimisation des seuils de détection dans le but de maximiser la probabilité de détection, présentée dans la \cref{sec:papier_appendix_A}, mais
%les faux positifs du jeu 1, avec des seuils plus restrictifs, sont aussi largement constitués de systèmes extratropicaux.
...
liés au gradient de géopotentiel par géostrophisme si bien que la hauteur géopotentielle à \hPa{200}, $Z_{200}$, s'effondre dans les moyennes latitudes. Cela
...
00:00, à \km{270} du cap Mizen à l'extrémité sud de l'Irlande, correspondant au point situé entre \ang{45} et \ang{50}N sur la \cref{fig:ophelia_z200}, cet
...
atteindre des latitues élevées si les conditions synoptiques le permettent, là où un seuil fixe en latitude aurait pu tronquer la trajectoire.
...
\hPa{200} étant directement proportionnelle à $\partial Z_{200} / \partial \phi$. Il en résulte que le lissage temporel préalable des champs concernés constitue
...
recouvrement de la trajectoire ERA5 avec celle d'IBTrACS. Comme défini dans la \cref{sec:papier_data_methods}, le taux de recouvrement correpond au nombre de
points ERA5 à moins \km{300} d'IBTrACS rapporté au nombre d'échéance de la trajectoire IBTrACS. Si en moyenne le taux de recouvrement des trajectoires appariées
(\textquote{\textit{matched}} dans \textcite{dulac_assessing_2023}) est de \prct{68} (médiane à \prct{71}), il suffit dans l'absolu d'un seul pas de temps
...
points ERA5 situés à plus de \km{300} des observations pour chaque paire de trajectoire (trajectoires correspondant au jeu 3 dans la
\cref{sec:filtrage_mid_latitudes}) et montre qu'en moyenne \prct{5.8} des points détectés par le traqueur parmi les trajectoires appariés à IBTrACS sont au delà
de la limite d'appariement. De plus, \prct{5} des trajectoires détectées dans ERA5 ont une fraction divergente d'au moins \prct{26}. Par conséquent, la
...
\begin{figure}[tb]
    ...
    \textbf{(b)} Histogramme de la fraction (en pourcentage) pour chaque trajectoire de points situés à plus de \km{300} de leur homologue dans IBTrACS, parmi
...
MAS$^\cup$ présente en effet une moyenne et une médiane toutes deux égales à \prct{42} ---~soit près de deux fois moins qu'avec MAS$^\cap$~--- tandis que
l'écart-type est de \prct{18}. En outre, cette métrique présente une relation très marquée avec $N_\ominus$, quasi-linéaire jusqu'à environ $N_\ominus = 20$, au
...
résolution horizontale de l'ordre de \km{25}. Les réanalyses constituent en effet la meilleure reconstruction tri-dimensionnelle de l'état passé de
