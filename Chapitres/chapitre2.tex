...
faible, vers une amélioration de la similarité, et plus prononcée dans le cas de MAS$^\cup$ que pour le wMAS. Ces deux distributions indiquent respectivement une
...
\prct{74} de la variance dans le cas de MAS$^\cup$, et \prct{72} dans le cas de wMAS~---~une fraction plus faible de la variance expliquée du $\Delta
\text{wMAS}$ par régression linéaire comparé au $\Delta \text{MAS}^\cup$ est cohérente avec le fait que la dépendance du wMAS avec $N_\ominus$ n'est pas
linéaire. Mais cela ne signifie pas pour autant que l'augmentation constatée dans ces deux métriques n'est qu'un artefact. En effet, rappelons que $N_\ominus$
mesure la différence dans la temporalité des deux trajectoires, si bien qu'une diminution du $\Delta N_\ominus$ indique que les trajectoires détectées dans le
schéma optimisé ont en général un début et une fin plus proche de la trajectoire observée. Ainsi, un $N_\ominus$ à la baisse avec un MAS$^\cap$ maintenu
constant constitue en soi une amélioration, et c'est ce que traduit l'information combinée de ces métriques. Par ailleurs, il est intéressant de constater sur
la seconde ligne de la \cref{fig:delta_MAS} que, pour les trois métriques considérées, le changement dans la similarité pour les cas où $\Delta N_\ominus = 0$
indique un changement moyen, certes extrêmement faible, mais systématiquement positif. En effet, le $\Delta \text{MAS}$ moyen pris à $\Delta N_\ominus = 0$ ($n
= 721$) vaut respectivement $+$\num{0.55} p.p, $+$\num{0.34} p.p et $+$\num{0.46} p.p, indiquant que l'optimisation du schéma de détection de cyclones tropicaux
