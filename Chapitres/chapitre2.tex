...
considérations sur le filtrage de systèmes de moyennes latitudes ---~source de biais important dans les schémas de détection de TC~--- et introduit trois
métriques visant à évaluer la ressemblance des trajectoires détectées dans ERA5 avec les trajectoires historiques
...
trajectoires $C \ominus Q$ contient les pas de temps pour lesquels une seule trajectoire existe à la fois.
...
%
\begin{equation}
    \bar{S_\theta}^\cap = \frac{1}{N_\cap} \sum_{i=1}^{N_\cap} S_\theta (\vec{c}_i, \vec{q}_i) \tag*{MAS$^\cap$}
\end{equation}
%
...
%
\begin{equation}
    \bar{S_\theta}^\cup = \frac{1}{N_\cup} \sum_{i=1}^{N_\cap} S_\theta (\vec{c}_i, \vec{q}_i) \tag*{MAS$^\cup$}
\end{equation}
%
...
%
\begin{equation*}
    \text{wMAS} = \frac{ w_1\sum_{i=1}^{N_\cap} S_{\theta}(\vec{c_i}, \vec{q_i}) }{ w_1 N_\cap + w_2 N_\ominus}\quad\quad \tag*{wMAS}
    \begin{cases}
...
\begin{figure}[htpb]
    ...
    \caption{Caractérisation de la relation entre la similarité des trajectoires et le nombre d'échéances \num{6}-horaires contenu dans $C \ominus Q$, pour les trois métriques
    MAS$^\cap$, MAS$^\cup$ et wMAS, dans lequel $a=1$ et $b=2$, évaluées sur \num{2244} paires de trajectoires ERA5 / IBTrACS et présentées dans les encadrés \textbf{(a)}, \textbf{(b)} et \textbf{(c)} respectivement.}
    \label{fig:MAS_Ndiff}
\end{figure}
%
...
exemples de la \cref{fig:exemple_MAS}. Le wMAS (\cref{fig:MAS_Ndiff} encadré \textbf{c}) vise donc à compenser cette propriété du MAS$^\cup$ via ses
...
distribution du wMAS est intermédiaire entre MAS$^\cap$ et MAS$^\cup$, avec \prct{59} en moyenne et \prct{62} en valeur médiane. Si on distingue toujours une
...
similarité des trajectoires, on calcule les valeurs des trois métriques de MAS pour les couples de trajectoires ERA5 / IBTrACS obtenus avant l'optimisation pour
les comparer aux similarités que les trajectoires actuelles présentent avec les mêmes trajectoires IBTrACS. Le jeu de trajectoires ERA5, dites
...
l'impact le plus prononcé étant sur les bassins NAtl et EPac. Cette diminution du POD avec ces trajectoires se traduit par un déficit de \num{465} couples ERA5
/ IBTrACS, puisque \num{1779} trajectoires ERA5 sont appariées avec ce jeu de données. Parmi ces paires de trajectoires, \num{1649} d'entre elles partagent la
même composante IBTrACS, signifiant que \num{130} paires sont manquantes dans les trajectoires dites \textquote{optimisées}. Cette perte peut s'expliquer par le
coût non-négligeable en POD du filtre de systèmes de moyennes latitudes utilisé dans \cite{dulac_assessing_2023}. En effet, le schéma de détection optimisé
consiste essentiellement en un relâchement des contraintes définissant un cyclone tropical, conduisant à un plus grand nombre de systèmes détectés et appariées,
notamment des systèmes peu intenses qui parfois ne présentent pas un cœur chaud suffisamment marqué selon le paramètre $V_U^T$ de \cite{hart_cyclone_2003}. Il
reste donc \num{1649} cyclones observés dans IBTrACS pour lesquelles nous disposons de deux trajectoires ERA5 issues de paramètres de détection différents. On
compare alors la similarité de chacune d'elles évaluée sur le cyclone observé, et pour les trois métriques de similarité présentées précédemment. La première
...
significativement différente de \num{0} avec test à \prct{95} (valeur-p $=$ \num{0.008}), ce changement est néanmoins insignifiant et peut s'expliquer par la
légère tendance à la baisse de $N_\ominus$, visible sur la seconde ligne de la \cref{fig:delta_MAS}. Cette baisse se traduit en effet par une augmentation
réciproque du $N^\cap$ moyen, si bien que la similarité angulaire moyennée sur $C \cap Q$ devient moins sensible aux petites incohérences locales entre les deux
trajectoires. Les deux métriques prenant en considération les échéances sur $C \ominus Q$ présentent quant à elles une tendance plus claire, bien que toujours
faible, vers une amélioration de la similarité, et plus prononcée dans le cas de MAS$^\cup$ que pour le wMAS. Ces deux distributions indiquent respectivement
une moyenne à $+$\num{2.4} p.p et $+$\num{2.05} p.p. Comme pour MAS$^\cap$, cette évolution est largement liée au changement de $N_\ominus$, avec une relation
linéaire évidente avec $\Delta N_\ominus$. Un ajustement linéaire sur cette quantité pour les deux métriques indique que le $\Delta N_\ominus$ explique
\prct{74} de la variance dans le cas de MAS$^\cup$ et \prct{72} dans le cas de wMAS. Mais cela ne signifie pas pour autant que l'augmentation constatée dans ces
deux métriques n'est qu'un artefact. En effet, rappelons que $N_\ominus$ mesure la différence dans la temporalité des deux trajectoires, si bien que $\Delta
N_\ominus < 0$ pour une même trajectoire IBTrACS indique que la trajectoire ERA5 issue du schéma optimisé présente un début et une fin plus proche de la
trajectoire observée. Ainsi, un $N_\ominus$ à la baisse avec un MAS$^\cap$ inchangé constitue en soi une amélioration de la ressemblance des trajectoires. Par
ailleurs, il est intéressant de constater sur la seconde ligne de la \cref{fig:delta_MAS} que, pour les trois métriques considérées, le changement dans la
similarité pour les cas où $\Delta N_\ominus = 0$ indique un changement moyen, certes extrêmement faible, mais systématiquement positif. En effet, le $\Delta
\text{MAS}$ moyen pris à $\Delta N_\ominus = 0$ ($n = 721$) vaut respectivement $+$\num{0.55} p.p, $+$\num{0.34} p.p et $+$\num{0.46} p.p. Cette hausse ne
saurait s'expliquer par la sensibilité de $\bar{S}_\theta^\cap$ à $N_\cap$, puisque ces cas correspondent à des trajectoires ERA5 définies exactement sur les
mêmes temps dans les deux jeux de données, et semble donc indiquer que l'optimisation du schéma de détection de cyclones tropicaux dans le but de maximiser la
probabilité de détection dans ERA5 par rapport à IBTrACS a pu, toutes choses égales par ailleurs, très légèrement améliorer la similarité des trajectoires
détectées avec celles observées, selon la définition de la similarité utilisée ici.
...
mais aussi que cette optimisation s'est accompagnée en moyenne d'une amélioration marginale. Cette dernière est en grande partie due au fait que la taille des
trajectoires détectées par le traqueur optimisé sont souvent plus proches de celles observées que celles issues du traqueur de référence, mais bien qu'une
diminution de $N_\ominus$ s'accompagne mécaniquement en moyenne d'une similarité accrue pour la raison évoquée précédemment, il est important de souligner que
l'augmentation équivalente de $N_\cap$ à similarité constante constitue également une amélioration. De plus, une petite amélioration est visible pour les cas où
la longueur est inchangée entre les deux jeux de données. L'analyse suggère par conséquent que l'impact de l'optimisation du traqueur sur la ressemblance des
